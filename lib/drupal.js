...
function systemConnect(success, failure) {
  ...
  xhr.onload = function (e) {
    ...
    console.log('systemConnect onload with status '+xhr.status);

    if (xhr.status == 200) {
      var response = xhr.responseText;
      var responseData = JSON.parse(response);

      console.log("systemConnect session " + responseData.sessid);
      console.log('systemConnect user ' + responseData.user.uid);
